 ...
 *
 * (1) Populate the local (`toCloud`) map with sensor data.
 ...
 *
 * There is no global state, we use `toCloud` and `fromCloud` (read-only) maps
 ...
 *  function(args: Array.<Object>, maps: {toCloud: {Object},
 * fromCloud: {Object}}, cb: caf.cb)
 *
 * where:
 *
 *  `args` contains custom arguments for the hook
 *  `maps` contains two objects: `toCloud` is a writeable map that
 *  will be uploaded to the CA  and `fromCloud` is a read-only map
 *  owned by the CA.
