...
var timeAdjuster = require('./TimeAdjuster');
...
var syncMaps = function(context, spec, maps, cb) {
    var $ = context || {};
    ...
    $.time && $.time.startRequest();
    request(req, function (err, response, body) {
                if (err) {
                    cb(err);
                } else {
                    $.time && $.time.endRequest(response);
...
// see MainLoop
var mainConstructor = function(context, spec, maps) {
    var $ = context;
    $.time = new timeAdjuster.TimeAdjuster(spec && spec.config
                                           && spec.config.smooth );
 ...
 *        smooth:number // optional exponential moving average factor to filter
 *                         time adjustments
 *     }
...
var MainLoop = exports.MainLoop = function(spec) {
    ...
    this.maps = {};
    this.$ = {};
    this.main = mainConstructor(this.$, spec, this.maps);
};
...
MainLoop.prototype.getTimeAdjuster = function() {
    return this.$.time;
};
